...
6. 对所有执行机构命令执行健康门控与饱和限制；
7. 输出控制指令并记录模式状态、控制残差、姿态误差与速度增量结果。
...
默认故障树扫掠覆盖无飞轮故障、任意单飞轮失效、任意双飞轮失效，叠加任意两台推进器降额至 40% 或任意两台推进器完全失效，并分别沿 $+X/+Y/+Z$ 执行轨控机动。对推进器完全失效后直接机体系方向不可达的工况，`sim_combined_fault.m` 先选取仍可由健康推进器实现的机体系燃烧方向，姿态机动对准同一惯性系 $\Delta v$ 方向后再执行轨控燃烧，并自动保留燃烧结束后的 20 s 评估窗口。完整矩阵共 4389 个工况，触发顶事件 1508 个，占 34.4%。其中姿态保持失败 1508 个，轨道机动失败 367 个，协同控制不可恢复 72 个；2394 个工况启用了推进器姿态辅助，33 个工况启用了先调姿再轨控恢复。
...
该结果说明：推进器降额通常不造成显著 $\Delta v$ 完成度损失，但会压缩姿态保持裕度；推进器完全失效导致部分机体系方向不可直接燃烧时，可通过先姿态机动、再沿健康推进器可达方向燃烧来恢复同一惯性系 $\Delta v$，但该策略依赖飞轮仍有足够姿态机动能力。双飞轮失效叠加推进器完全失效时，推进器同时承担轨控与缺失力矩补偿，顶事件比例最高，是故障树中的关键组合事件。
...
4. 组合故障树结果表明，“双飞轮失效 + 推进器完全失效”是顶事件比例最高的关键组合，应作为后续任务管理和故障处置策略的重点；
5. 本轮交付成果具备继续开展工程化深化和半实物验证的基础。
