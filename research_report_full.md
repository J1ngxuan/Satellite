...
2. 陀螺测量包含白噪声和常值偏置；
3. 飞轮和推进器故障状态由外部故障检测模块给定；
4. 本项目聚焦“故障已知条件下的容错控制”，不扩展至故障诊断算法开发。
...
该方案在六个主轴方向各布置两台共线喷嘴，并通过对称安装实现标称工况下的寄生力矩内消。方案特点如下：
...
该流程各步骤边界清晰，便于后续接入故障诊断、任务管理和上层制导模块。
...
|---|---:|---:|---|
| 无故障 | 0.0757° | 0.0846° | 纯飞轮 |
| 1 台飞轮失效 | 0.0675° | 0.0755° | 纯飞轮 |
| 2 台飞轮失效 | 0.0722° | 0.0858° | 飞轮 + 推进器协同 |
...
1. 单飞轮失效不会导致姿态性能显著退化；
2. 双飞轮失效时，系统通过协同控制维持了约 `0.1°` 量级姿态保持精度；
...
|---|---:|---:|---:|
| [1 2] | 3.2813° | 0.0747° | 0.0682° |
| [1 3] | 23.7105° | 0.0755° | 0.0752° |
| [1 4] | 79.2843° | 0.0642° | 0.0762° |
| [2 3] | 25.2398° | 0.0625° | 0.0691° |
| [2 4] | 61.2277° | 0.0514° | 0.0553° |
| [3 4] | 37.7165° | 0.0513° | 0.0588° |

![双飞轮失效策略对比](./figures/double_wheel_strategy_sweep.png)

扫描结果表明：在当前初始姿态偏差、角速度、传感器误差和外扰设定下，单纯两飞轮投影控制无法满足 0.1° 姿态保持指标；`wheel_first` 策略均在 30 s 评估窗后启用推力器兜底，并恢复到 0.05°–0.08° 量级。因此，本项目对参考论文的采用方式不是取消推力器协同，而是将其前置为“双飞轮能力评估 + 精度不足再兜底”的控制逻辑。
...
$$
\theta_{\mathrm{peak,ff}} = 0.1000^\circ
$$

不启用飞轮前馈补偿时，姿态误差峰值为：

$$
\theta_{\mathrm{peak,noff}} = 51.9280^\circ
$$
...
默认故障树扫掠覆盖无飞轮故障、任意单飞轮失效、任意双飞轮失效，叠加任意两台推进器降额至 40% 或任意两台推进器完全失效，并分别沿 $+X/+Y/+Z$ 执行轨控机动。对推进器完全失效后直接机体系方向不可达的工况，`sim_combined_fault.m` 先选取仍可由健康推进器实现的机体系燃烧方向，姿态机动对准同一惯性系 $\Delta v$ 方向后再执行轨控燃烧，并自动保留燃烧结束后的 20 s 评估窗口。进一步地，轨控燃烧过程中若残余寄生力矩超过飞轮可补偿裕度，算法会降低当前步推力并采用慢速机动完成目标速度增量。完整矩阵共 4389 个工况，触发顶事件 1412 个，占 32.2%。其中姿态保持失败 1412 个，轨道机动失败 253 个，协同控制不可恢复 26 个；2394 个工况启用了推进器姿态辅助，33 个工况启用了先调姿再轨控恢复，400 个工况触发了低于满推力的慢速机动。

| 飞轮失效数 | 推进器模式 | 工况数 | 顶事件比例 | 最大稳态姿态误差 | 最大 $\Delta v$ 误差 |
|---:|---|---:|---:|---:|---:|
| 0 | nominal | 3 | 0.0% | 0.0983° | 2.04% |
| 0 | degrade | 198 | 10.1% | 0.1172° | 2.09% |
| 0 | failure | 198 | 10.1% | 0.1292° | 3.97% |
| 1 | nominal | 12 | 0.0% | 0.0991° | 2.07% |
| 1 | degrade | 792 | 12.0% | 0.1295° | 2.11% |
| 1 | failure | 792 | 15.4% | 0.1361° | 4.13% |
| 2 | nominal | 18 | 33.3% | 0.1562° | 2.22% |
| 2 | degrade | 1188 | 34.3% | 0.1734° | 2.27% |
| 2 | failure | 1188 | 62.4% | 104.6420° | 130.62% |

![组合故障树扫掠热力图](./figures/fault_tree_combined_sweep.png)

该结果说明：推进器降额通常不造成显著 $\Delta v$ 完成度损失，慢速机动策略可消除一部分由残余力矩裕度不足引起的姿态越限；推进器完全失效导致部分机体系方向不可直接燃烧时，可通过先姿态机动、再沿健康推进器可达方向燃烧来恢复同一惯性系 $\Delta v$，但该策略依赖飞轮仍有足够姿态机动能力。双飞轮失效叠加推进器完全失效时，推进器同时承担轨控与缺失力矩补偿，部分故障几何即使降至 20% 推力仍无法同时维持指向和完成 $\Delta v$，仍是故障树中的关键组合事件。

#### 13.6.1 典型顶事件原因分析

为说明顶事件不是单一阈值判定结果，而是由执行机构可达性、分配约束和姿态/轨道任务耦合共同触发，选取以下代表性工况进行原因分析。其中部分工况在引入残余力矩裕度检查和慢速机动后已不再触发顶事件，说明原有失败并非不可恢复硬故障，而是固定推力燃烧策略下的控制裕度不足。

| 代表工况 | 判定结果 | 主要现象 | 原因分析 |
|---|---|---|---|
| 无飞轮失效，推进器 `#9/#10` 降额，沿 `+X` 机动 | 未触发顶事件 | 平均姿态误差约 `0.0877°`，$\Delta v$ 误差约 `2.05%` | 降额喷嘴造成的残余力矩仍在飞轮可补偿范围内，算法保持满推力燃烧即可稳定完成任务 |
| 1 号飞轮失效，推进器 `#6/#8` 完全失效，沿 `+Y` 机动 | 未触发顶事件 | 平均姿态误差约 `0.0996°`，$\Delta v$ 误差约 `1.22%`，燃烧时间延长至约 `69.5 s`，最低推力比例为 `0.40` | 原固定推力下残余力矩接近飞轮裕度边界；算法识别飞轮补偿余量不足后降低推力，使轨控合力减小、寄生力矩同步下降，以较长燃烧时间换取稳定控制 |
| 1、3 号飞轮失效，推进器标称，沿 `+X` 机动 | 未触发顶事件 | 平均姿态误差约 `0.0939°`，$\Delta v$ 误差约 `2.06%` | 双飞轮失效使飞轮可达力矩空间降秩，但标称推进器仍可提供姿态辅助力矩，协同控制后姿态误差低于阈值 |
| 1、3 号飞轮失效，推进器 `#3/#11` 降额，沿 `+X` 机动 | 姿态保持失败 | 平均姿态误差约 `0.1032°`，$\Delta v$ 误差约 `2.08%` | 降额压缩了推进器力矩辅助余量，剩余喷嘴集合仍可完成 $\Delta v$，但燃烧后姿态保持裕度略低于 `0.1°` 指标 |
| 2、3 号飞轮失效，推进器 `#1/#6` 完全失效，沿 `+Y` 机动 | 姿态保持失败 + 轨道机动失败 | 平均姿态误差约 `90.0104°`，$\Delta v$ 误差约 `100.09%`，燃烧达到 `200 s` 慢速上限，最低推力比例为 `0.20` | 该组合同时削弱飞轮可达力矩子空间和 `+Y` 方向推进器可达性；即使降至最低推力，剩余执行机构仍无法提供足够姿态控制自由度，姿态偏离导致惯性系 $\Delta v$ 投影严重下降 |
| 1、4 号飞轮失效，推进器 `#2/#6` 完全失效，沿 `+Y` 机动 | 姿态保持失败 + 轨道机动失败 | 平均姿态误差约 `91.3827°`，$\Delta v$ 误差约 `98.80%`，燃烧达到 `200 s` 慢速上限，最低推力比例为 `0.20` | 推进器需要同时承担轨控合力和双飞轮失效后的缺失力矩补偿，剩余喷嘴自由度不足，慢速降推力只能避免继续扩大饱和风险，不能恢复不可达方向上的姿态控制能力 |

上述案例表明，顶事件的主要成因可归纳为三类：

1. **裕度型越限可通过慢速机动缓解**：推进器降额或单飞轮失效主要造成残余力矩裕度下降，降低推力后寄生力矩随之降低，部分工况可恢复稳定。
2. **几何可达性丧失仍是硬约束**：推进器完全失效若切断目标方向和姿态补偿方向的关键喷嘴组合，即使降低推力也不能恢复缺失的控制自由度。
3. **协同控制资源冲突是剩余顶事件主因**：双飞轮失效与推进器完全失效叠加时，推进器既要承担轨控合力，又要补偿飞轮缺失力矩；当剩余喷嘴集合无法提供足够独立自由度时，顶事件仍会由姿态和轨道分支共同触发。
...
| 单/双飞轮故障下姿态稳定控制 | 双飞轮故障场景平均误差 `0.0722°`；新增两飞轮优先评估，精度不足时由推力器兜底 | 满足 |
| 多台推进器推力下降下轨道机动能力 | 推力下降场景实现 `0.3000 m/s` 目标速度增量 | 满足 |
| 多台推进器失效下轨道机动能力 | 失效场景实现 `0.2957 m/s` 速度增量 | 满足 |
| 组合故障覆盖性分析 | 新增 4389 个飞轮/推进器组合工况故障树扫掠，识别关键组合事件 | 满足 |
...
4. 新增残余力矩裕度检查与慢速机动后，组合故障树中部分原顶事件可通过降低推力和延长燃烧时间恢复，更新后的完整矩阵顶事件比例为 `32.2%`；
5. 组合故障树结果表明，“双飞轮失效 + 推进器完全失效”仍是顶事件比例最高的关键组合，应作为后续任务管理和故障处置策略的重点；
