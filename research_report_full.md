...
默认故障树扫掠覆盖无飞轮故障、任意单飞轮失效、任意双飞轮失效，叠加任意两台推进器降额至 40% 或任意两台推进器完全失效，并分别沿 $+X/+Y/+Z$ 执行轨控机动。完整矩阵共 4389 个工况，触发顶事件 1507 个，占 34.3%。其中姿态保持失败 1486 个，轨道机动失败 85 个，协同控制不可恢复 46 个；2394 个工况启用了推进器姿态辅助。
...
该结果说明：推进器降额通常不造成 $\Delta v$ 完成度损失，但会压缩姿态保持裕度；推进器完全失效在部分方向上会直接造成轨控能力缺失。双飞轮失效叠加推进器完全失效时，推进器同时承担轨控与缺失力矩补偿，顶事件比例最高，是故障树中的关键组合事件。
