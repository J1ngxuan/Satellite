...
默认故障树扫掠覆盖无飞轮故障、任意单飞轮失效、任意双飞轮失效，叠加任意两台推进器降额至 40% 或任意两台推进器完全失效，并分别沿 $+X/+Y/+Z$ 执行轨控机动。对推进器完全失效后直接机体系方向不可达的工况，`sim_combined_fault.m` 先选取仍可由健康推进器实现的机体系燃烧方向，姿态机动对准同一惯性系 $\Delta v$ 方向后再执行轨控燃烧，并自动保留燃烧结束后的 20 s 评估窗口。进一步地，轨控燃烧过程中若残余寄生力矩超过飞轮可补偿裕度，算法会降低当前步推力并采用慢速机动完成目标速度增量。完整矩阵共 4389 个工况，触发顶事件 284 个，占 6.5%。其中姿态保持失败 284 个，轨道机动失败 229 个，协同控制不可恢复 19 个；2394 个工况启用了推进器姿态辅助，33 个工况启用了先调姿再轨控恢复，630 个工况触发了低于满推力的慢速机动。

| 飞轮失效数 | 推进器模式 | 工况数 | 顶事件比例 | 最大稳态姿态误差 | 最大 $\Delta v$ 误差 |
|---:|---|---:|---:|---:|---:|
| 0 | nominal | 3 | 0.0% | 0.0373° | 0.61% |
| 0 | degrade | 198 | 0.0% | 0.0474° | 0.63% |
| 0 | failure | 198 | 0.0% | 0.0463° | 1.20% |
| 1 | nominal | 12 | 0.0% | 0.0379° | 0.62% |
| 1 | degrade | 792 | 0.0% | 0.0523° | 0.64% |
| 1 | failure | 792 | 0.0% | 0.0470° | 0.94% |
| 2 | nominal | 18 | 0.0% | 0.0410° | 0.63% |
| 2 | degrade | 1188 | 0.0% | 0.0496° | 0.64% |
| 2 | failure | 1188 | 23.9% | 99.9517° | 158.00% |

![组合故障树扫掠热力图](./figures/fault_tree_combined_sweep.png)

#### 13.6.1 典型顶事件分布

改进后的控制增益与喷嘴布局将顶事件基本压缩到“双飞轮失效 + 推进器完全失效”这一组合子空间。按飞轮失效阶次与推进器模式汇总如下：

| 飞轮失效数 | 推进器模式 | 工况数 | 顶事件率 | 最大稳态姿态误差 | 最大 $\Delta v$ 误差 | 助推使用率 |
|---:|---|---:|---:|---:|---:|---:|
| 0 | nominal | 3 | 0.0% | 0.0373° | 0.61% | 0.0% |
| 0 | degrade | 198 | 0.0% | 0.0474° | 0.63% | 0.0% |
| 0 | failure | 198 | 0.0% | 0.0463° | 1.20% | 0.0% |
| 1 | nominal | 12 | 0.0% | 0.0379° | 0.62% | 0.0% |
| 1 | degrade | 792 | 0.0% | 0.0523° | 0.64% | 0.0% |
| 1 | failure | 792 | 0.0% | 0.0470° | 0.94% | 0.0% |
| 2 | nominal | 18 | 0.0% | 0.0410° | 0.63% | 100.0% |
| 2 | degrade | 1188 | 0.0% | 0.0496° | 0.64% | 100.0% |
| 2 | failure | 1188 | 23.9% | 99.9517° | 158.00% | 100.0% |

上述结果说明：

1. 推进器降额在当前控制律下仍可维持亚 `0.05°` 量级的末段姿态误差，且 `\Delta v` 误差保持在 `1.2%` 以内；
2. 双飞轮失效但推进器标称或降额时，系统多数工况可通过姿态先调与慢速机动恢复；
3. 只有“双飞轮失效 + 推进器完全失效”同时出现时，剩余执行机构自由度才显著不足，成为当前版本的主要顶事件来源；
4. 相比旧版 `32.2%` 顶事件率，当前版本已将风险面压缩到 `6.5%`。
...
| 组合故障覆盖性分析 | 新增 4389 个飞轮/推进器组合工况故障树扫掠，顶事件率为 `6.5%` | 满足 |
...
4. 新增残余力矩裕度检查与慢速机动后，组合故障树中部分原顶事件可通过降低推力和延长燃烧时间恢复，更新后的完整矩阵顶事件比例为 `6.5%`；
5. 组合故障树结果表明，“双飞轮失效 + 推进器完全失效”仍是当前版本唯一显著的顶事件组合，应作为后续任务管理和故障处置策略的重点；
